...
def get_book(book_id):
    ...
    conn = Datalayer().conn
    book = conn.execute("SELECT * FROM Libraries WHERE id = ?", (book_id,)).fetchone()
    conn.close()
    if book is None:
        abort(404)
    return book
...
def add_new_book_to_liblary(
    ...
    conn = Datalayer().conn
    conn.execute(
        "INSERT INTO Libraries (title, authors, publishedDate, ISBN, pagesCount, previewLink, languages) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def edit_book_by_id(
    ...
    conn = Datalayer().conn
    conn.execute(
        "UPDATE Libraries SET title = ?, authors = ?, publishedDate = ?, ISBN = ?, pagesCount = ?, previewLink = ?, languages = ?"
        " WHERE id = ?",
        (title, authors, publishedDate, ISBN, pagesCount, previewLink, languages, id),
    )
    conn.commit()
    conn.close()
    return 200
...
def search_in_author(author: str):
    conn = Datalayer().conn
    book = conn.execute(
        "SELECT * FROM Libraries WHERE authors = ?", (author,)
    ).fetchall()
    conn.close()
    return book
...
def search_in_language(language: str):
    conn = Datalayer().conn
    book = conn.execute(
        "SELECT * FROM Libraries WHERE languages = ?", (language,)
    ).fetchall()
    conn.close()
    return book
...
def search_in_year(date: list):
    conn = Datalayer().conn
    book = conn.execute(
        "SELECT * FROM Libraries WHERE publishedDate BETWEEN ? AND ?", (date)
    ).fetchall()
    conn.close()
    return book
